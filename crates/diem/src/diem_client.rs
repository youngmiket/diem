use diem_client::{views, BlockingClient, Response, WaitForTransactionError, Client};
...
impl DiemClient {
    ...
    pub async fn get_account(&self, account: &AccountAddress) -> Result<Option<views::AccountView>> {
        self.client
    ...
    pub async fn get_txn_by_acc_seq(
        ...
        self.client
            .get_account_transaction(*account, sequence_number, fetch_events).await
            .map_err(Into::into)
            .map(Response::into_inner)
    }
    ...
    pub async fn wait_for_transaction(
        ...
        self.client
            .wait_for_signed_transaction(txn, Some(timeout), None).await
            .map(Response::into_inner)
    }
